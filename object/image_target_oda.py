...
def train_target(args, zz=''):
    ...
    for epoch in tqdm(range(args.max_epoch), leave=False):
        ...
        log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}% / {:.2f}% / {:.2f}%'.format(args.name, epoch+1, args.max_epoch, acc_os2, acc_os1, acc_unknown)
        args.out_file.write(log_str + '\n')
        args.out_file.flush()
        print(log_str+'\n')
    
    # torch.save(netF.state_dict(), osp.join(args.output_dir, 'target_F' + str(args.cls_par) + '.pt'))
    # torch.save(netB.state_dict(), osp.join(args.output_dir, 'target_B' + str(args.cls_par) + '.pt'))
    # torch.save(netC.state_dict(), osp.join(args.output_dir, 'target_C' + str(args.cls_par) + '.pt'))
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    # known_idx = np.where(kmeans.labels_ == 0)[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--da', type=str, default='oda', choices=['uda', 'pda', 'oda', 'opda'])
